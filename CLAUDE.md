...
### Google Ads 現状
- **未導入**（Google Ads タグ、コンバージョントラッキングなし）
- firefitnessでは `G-67PC6X0LHN`（Ads Tag）+ GTMサーバーコンテナ経由CAPI連携
- cycleZで導入する場合:
  1. Google Ads アカウント作成
  2. コンバージョンアクション設定（お問い合わせ、電話クリック）
  3. Ads Tag IDを`Analytics.tsx`に追加
  4. GTM導入を検討（サーバーサイドタグはオーバースペックの可能性）
