...
### 取扱ブランド（承認済み）
GIOS, BASSO, SCOTT, BOMA, BISYA, De Rosa, ORBEA, FELT, CERVELO, CINELLI, Wilier, CYCLEHEART, SURLY, JAMIS, Tyrell, macchi cycles, MATE.BIKE
...
### A2. 決め台詞（2026-08-22 追加・全媒体共通）
- [ ] **冒頭に1文・締めに1文だけ**、語尾が「**〜だゼーーーット！！**」になっているか？（伸ばし棒3つ・全角！2つ）
- [ ] 本文中に3回目が出ていないか？（計2文まで。乱発禁止）
- [ ] 締めの1文はCTA（電話番号）と**同じ最終ブロック**に置いたか？（離すと D. のCTA1回ルールに別CTAとして引っかかる）
- [ ] 休業・お詫び等の事務連絡記事では**入れていない**か？
- ※ 対象はブログだけでなく Instagramキャプション／Threads／リールのキャプション／GBP投稿（GBPは締め1文のみ）も共通。詳細は親の `12_cycleZ/CLAUDE.md`「cycleZ 決め台詞ルール」
...
1. **一次情報メモを読み込み**（CRMから取得）
   ```bash
   node ~/開発プロジェクト/12_cycleZ/cyclez-crm/scripts/fetch-blog-notes.mjs
   ```
...
3. **使用した一次情報メモのステータスを更新**
   ```bash
   node ~/開発プロジェクト/12_cycleZ/cyclez-crm/scripts/fetch-blog-notes.mjs --mark-used <使用したメモID>
   ```
